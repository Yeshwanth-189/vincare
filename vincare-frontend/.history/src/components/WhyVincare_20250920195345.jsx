...
export default function WhyVincare() {
  return (
    // src/components/WhyVincare.jsx (render part only)
    <section className="why grid-12">
      ...
      <div className="whyRows grid-12">
        {items.map((it, idx) => {
          const odd = (idx + 1) % 2 === 1; // 1st, 3rd, 5th rows
          return (
            <div className="whyRow" key={it.title}>
              {/* Image (left on odd rows, right on even rows) */}
              <div className={`whyImg ${odd ? "imgLeft" : "imgRight"}`}>
                <img src={it.img} alt={it.title} loading="lazy" />
              </div>

              {/* Text (right on odd rows, left on even rows) */}
              <div className={`whyText ${odd ? "textRight" : "textLeft"}`}>
                <h3>{it.title}</h3>
                <p>{it.text}</p>
              </div>
            </div>
