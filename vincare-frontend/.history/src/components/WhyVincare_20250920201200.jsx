  ...
  return (
    ...
    <section className="why grid-12">
      ...
      <div className="why__rows">
        {items.map((it, idx) => {
          const isLeft = idx % 2 === 0; // alternate sides per row
          return (
            <motion.div
              key={it.title}
              className="why__row"
            ...
            >
              ...
              <div className={`why__cardWrap ${isLeft ? "left" : "right"}`}>
                <article
                  className={`why__card ${isLeft ? "" : "why__card--reverse"}`}
                >
              ...
              {/* NEW: image on the opposite (empty) side */}
              <div className={`why__imgCol ${isLeft ? "right" : "left"}`}>
                <img src={it.img} alt={it.title} loading="lazy" />
              </div>

              {/* center badge (unchanged) */}
